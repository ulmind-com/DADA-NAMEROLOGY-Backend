...
def _issue_session(db: Session, user: User, device: str = "") -> TokenOut:
...
def signup_start(body: EmailIn, db: Session = Depends(get_db)):
    ...
    data = issue_otp(db, body.email, OtpPurpose.signup)
    return OtpStartOut(**data)


@router.post("/signup/resend", response_model=OtpStartOut, summary="Resend the signup OTP")
def signup_resend(body: EmailIn, db: Session = Depends(get_db)):
    if _get_user_by_email(db, body.email):
...
def signup_verify(body: OtpVerifyIn, db: Session = Depends(get_db)):
...
def signup_complete(body: SignupCompleteIn, db: Session = Depends(get_db)):
...
def google_auth(body: GoogleIn, db: Session = Depends(get_db)):
    ...
    user = db.scalars(select(User).where(User.google_id == info["google_id"])).first()
    if not user:
        user = _get_user_by_email(db, info["email"])
        if user:
            user.google_id = info["google_id"]
            user.avatar_url = user.avatar_url or info["picture"]
...
def forgot_verify(body: OtpVerifyIn, db: Session = Depends(get_db)):
...
def forgot_reset(body: ResetIn, db: Session = Depends(get_db)):
